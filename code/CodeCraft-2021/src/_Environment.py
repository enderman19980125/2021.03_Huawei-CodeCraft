...
class Environment:
    ...
    def get_vm_config_by_id(self, vm_config_id: str) -> VMConfig:
        vm_config = self.__vm_config_dict.get(vm_config_id)
        if vm_config is None:
            raise KeyError(f"The VMConfig[{vm_config_id}] doesn't exist.")
        return vm_config

    def get_vm_by_id(self, vm_id: str) -> Union[SingleVM, DoubleVM]:
        vm = self.__vm_dict.get(vm_id)
        if vm is None:
    ...
    def get_vm_dict(self) -> Dict[str, Union[SingleVM, DoubleVM]]:
    ...
    def start_next_day(self) -> None:
        ...
        self.__current_day_operation = DayOperation(day=self.get_current_day())

    def add_vm_operation(self, day: int, op: Union[DeploySingleVMOperation, DeployDoubleVMOperation,
    ...
    def purchase_server(self, server_config: ServerConfig) -> Server:
    ...
    def deploy_single_vm(self, vm: SingleVM, server: Server, node: str) -> None:
        if vm.get_vm_id() in self.__vm_dict.keys():
            raise KeyError(f"The VM[{vm.get_vm_id}] already exists.")
        self.__vm_dict[vm.get_vm_id] = vm
        server.deploy_single_vm_with_check(vm=vm, node=node)
        vm.deploy(server=server, node=node)

        op = DeploySingleVMOperation(day=self.get_current_day(), vm=vm, server=server, node=node)
        vm.add_operation(op=op)
        self.__current_day_operation.add_deploy_vm_operation(op=op)

    def deploy_double_vm(self, vm: DoubleVM, server: Server) -> None:
        if vm.get_vm_id() in self.__vm_dict.keys():
            raise KeyError(f"The VM[{vm.get_vm_id}] already exists.")
        self.__vm_dict[vm.get_vm_id] = vm
        ...
        op = DeployDoubleVMOperation(day=self.get_current_day(), vm=vm, server=server)
        vm.add_operation(op=op)
        self.__current_day_operation.add_deploy_vm_operation(op=op)

    def migrate_single_vm(self, vm: SingleVM, new_server: Server, node: str) -> None:
    ...
    def migrate_double_vm(self, vm: DoubleVM, new_server: Server) -> None:
        if vm.get_vm_id() not in self.__vm_dict.keys():
            raise KeyError(f"The VM[{vm.get_vm_id}] doesn't exist.")
    ...
    def remove_vm(self, vm: Union[SingleVM, DoubleVM]) -> None:
        ...
        if vm.is_double():
            op = RemoveDoubleVMOperation(day=self.get_current_day(), vm=vm)
        else:
            op = RemoveSingleVMOperation(day=self.get_current_day(), vm=vm)
        vm.add_operation(op=op)
        self.__current_day_operation.add_remove_vm_operation(op=op)
